...
fort_tas <- fortify_sfc(tas_sa2, keep = 0.1)
```
...
sa2_map <- create_hexmap(shp = tas_sa2, 
  ...
  hex_filter = 10, hex_size = 0.27, export_shp = FALSE, 
...
```{r}
hex_points_df <- fortify_hexagon(data = hexmap_sa2, sf_id = "SA2_NAME16", hex_size = 0.27)
...
hex <- ggplot(hex_animate, aes(group = interaction(SA2_NAME16,polygon))) + 
  geom_polygon(aes(x = long, y = lat, group = interaction(SA2_NAME16, polygon), fill = SA4_NAME16)) +  coord_equal() + invthm +
...
animate(hex, nframes=20, duration = 10, bg = 'transparent', renderer = gifski_renderer("tasanim.gif"))
animate(hex, nframes=20, duration = 10, bg = 'transparent', device = "png", 
