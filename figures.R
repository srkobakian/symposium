...
legend_b <- get_legend(sa4_map)

aus_sa_areas <- plot_grid(
  plot_grid(
  sa4_map +
    scale_fill_distiller(type = "seq", palette = "BuGn",  
      direction = 1, na.value = "light grey",
  limits = c(min(sa2$albers_sqkm), max(sa4$albers_sqkm))) + guides(fill = FALSE),
  sa3_map +
    scale_fill_distiller(type = "seq", palette = "BuGn",  
      direction = 1, na.value = "light grey",
      limits = c(min(sa2$albers_sqkm), max(sa4$albers_sqkm))) + guides(fill = FALSE),
  sa2_map +
    scale_fill_distiller(type = "seq", palette = "BuGn",  
      direction = 1, na.value = "light grey",
      limits = c(min(sa2$albers_sqkm), max(sa4$albers_sqkm))) + guides(fill = FALSE), nrow = 1), legend_b, ncol = 1, rel_heights = c(1, .1))


ggsave(filename = "figures/aus_sa2.png", plot = aus_sa_areas,
  device = "png", bg = "transparent", dpi = 300,  width = 10, height = 8)
